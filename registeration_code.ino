...
const int buzzerPin = D3;  // Replace with the actual pin connected to the buzzer
...
void loop() {
  ...
  if (rfid.PICC_ReadCardSerial()) {
    ...
    u8g2.drawStr(0,10,"Welcome");  // write something to the internal memory
    u8g2.drawStr(0,30,"process in App");  // write something to the internal memory
    u8g2.sendBuffer(); 
    delay(100);
    ...
    if (Firebase.RTDB.setString(&fbdo, "smarttravel/add", tag)){
    ...
    if (Firebase.RTDB.getString(&fbdo, "/smarttravel/result"))
    {
      ...
      if (mySubString == "a")
      {
        u8g2.clearBuffer();          // clear the internal memory
        u8g2.setFont(u8g2_font_ncenB08_tr); // choose a suitable font
        u8g2.drawStr(0,10,"User Added");  // write something to the internal memory
      ...
      else if (mySubString == "b")
      {
        u8g2.clearBuffer();          // clear the internal memory
        u8g2.setFont(u8g2_font_ncenB08_tr); // choose a suitable font
        u8g2.drawStr(0,10,"Amount added");  // write something to the internal memory
        u8g2.sendBuffer();          // transfer internal memory to the display
        delay(5000); 
        u8g2.clearBuffer();          // clear the internal memory
        u8g2.sendBuffer(); 
